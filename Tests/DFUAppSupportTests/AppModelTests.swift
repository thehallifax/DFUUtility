...
@Test func mainLayoutKeepsHeaderOutsideScrollableBoundedWorkspace() throws {
    ...
    #expect(about.contains("View DFUUtility License")); #expect(about.contains("View macvdmtool License"))
...
@Test func visualParityUsesSharedPanelsAndResponsiveActions() throws {
    ...
    #expect(content.contains("HStack(alignment: .top, spacing: 16)"))
    #expect(content.contains("WorkspacePanel(\"Firmware for this device\""))
    ...
    #expect(capture.contains("tableHeight")); #expect(capture.contains("WorkspacePanel(\"Asset Tag\"")); #expect(capture.contains("identifierBlock"))
    #expect(capture.contains("ScrollView([.horizontal, .vertical])"))
    #expect(capture.contains("help: record.serialNumber")); #expect(capture.contains("help: record.ecid")); #expect(capture.contains("identifierContextMenu"))
    #expect(capture.contains("textSelection(.enabled)"))
    #expect(capture.contains("design: .monospaced")); #expect(capture.contains("Button(\"Copy All\")"))
    #expect(capture.contains("showFullIdentifiers")); #expect(capture.contains("showFullIdentifiers ? record.serialNumber : record.shortSerial")); #expect(capture.contains("showFullIdentifiers ? record.ecid : record.shortECID"))
    #expect(capture.contains("let serialWidth: CGFloat = showFullIdentifiers ? 140 : 95"))
    #expect(content.contains("DeviceCaptureView(model: model)\n                .padding(24)"))
}
