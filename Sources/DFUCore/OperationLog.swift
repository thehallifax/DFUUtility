...
public final class OperationLogger: @unchecked Sendable, OperationLogging {
    ...
    public func start(operation: String, target: DFUDevice?, release: IPSWRelease?) throws -> URL {
        ...
        let header = "Timestamp: \(formatter.string(from: Date()))\nDFUUtility: \(BuildMetadata.displayVersion)\nGit commit: \(BuildMetadata.gitCommit)\nOperation: \(operation)\nTarget family: \(target?.family.displayName ?? "Unknown")\nTarget product: \(target?.restoreProductType ?? "Unknown")\nTarget ECID: \(target?.ecid ?? "Unknown")\nIPSW: \(release.map { "\($0.platform.displayName) \($0.version) (\($0.build))" } ?? "Not applicable")\n\n"
